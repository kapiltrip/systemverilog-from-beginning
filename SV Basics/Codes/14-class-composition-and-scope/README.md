...
[← Part 13](../13-constructor-arguments/README.md) · [Learning index](../README.md) · [Part 15 →](../15-class-shallow-copy/README.md)
...
Composition means one object owns or refers to another object as part of its state. The outer handle and nested handle still refer to separate heap objects, so constructing the outer object does not automatically construct every nested object unless its constructor does that work.

Setter and getter methods are useful here because they make access policy visible. Follow the handle chain in the code and note exactly where each `new` occurs; that reveals which object owns each piece of state and where a null handle could appear.
