# scripts/generate_full_revision_md.py
"""
Generates the complete Master SystemVerilog Revision Document in Markdown.
Follows strict user preferences:
- AVOID TABLES: Tables look diluted. Only use the single authoritative comparison
  table for Integral Data Types where contrasting states, widths, signedness,
  and defaults is super important.
- All other topics use indexed headlines, structured sections, rich, well-mannered
  technical paragraphs, verbatim "How to Speak It" interview guides, and numbered
  bug audit dossiers.
- Incorporates all previous questions from GitHub Lessons 01-21 and laboratory practice C1-C9 & OOP.
- Fully incorporates Constrained Randomization fundamentals, error handling, and lifecycle
  from the newly supplied practice snippet (rand, assert(randomize()), !(inside), declaration order).

Outputs to:
- docs/SystemVerilog_Data_Types_and_OOP_Revision.md
- SV Basics/Revision/Data-Types-and-OOP-Revision.md
"""
...
MD_DOCS = DOCS_DIR / "SystemVerilog_Data_Types_and_OOP_Revision.md"
MD_REV = REV_DIR / "Data-Types-and-OOP-Revision.md"

def get_front_matter():
    return r"""# SystemVerilog Unified Revision Guide: Data Types, OOP, and Testbench Architecture
...
def get_matrix():
    return r"""## Master Cross-Revision Question & Knowledge Guide
...
def get_part_1():
    return r"""## Part I: Data Types, Representation & Simulation Foundations
...
def get_part_2():
    return r"""## Part II: Subroutines & Argument Passing Semantics
...
def get_part_3():
    return r"""## Part III: Object-Oriented Programming (OOP) in SystemVerilog
...
def get_part_4_to_6():
    return r"""## Part IV: Practice Code Bug Audit and Code Repairs
...
def generate_markdown():
    ...
    MD_DOCS.write_text(full_text, encoding="utf-8")
    MD_REV.write_text(full_text, encoding="utf-8")
    print(f"Markdown generated successfully: {len(full_text.split())} words, {len(full_text)} characters.")
    print(f"- Output 1: {MD_DOCS}")
    print(f"- Output 2: {MD_REV}")
