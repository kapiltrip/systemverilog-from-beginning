...
[← Part 27](../27-runtime-constraint-range-changes-with-randc/README.md) · [Learning index](../README.md) · [Part 29 →](../29-distribution-constraints-with-colon-equal-and-colon-slash/README.md)
...
This part combines several solver controls: implication for conditional legality, equivalence for a two-way relationship, `dist` for weighted choice, and constraint modes for enabling or disabling a named block. Each answers a different modeling question.

Read them in that order: determine what values are legal, determine conditional relationships, then consider relative likelihood, and finally check which constraint blocks are active. Probability cannot make an illegal value legal, and a disabled constraint contributes no restriction.
