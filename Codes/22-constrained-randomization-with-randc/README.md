...
[← Part 21](../21-constructor-arguments-and-super-keyword/README.md) · [Learning index](../README.md) · [Part 23 →](../23-constrained-randomization-with-a-single-constraint/README.md)
...
This is the transition from manually chosen values to solver-generated stimulus. `randc` asks for cyclic random behavior, the constraint narrows the legal solution space, and `randomize()` reports whether the solver found a legal assignment.

For the active 4-bit `a > 10` constraint, first enumerate the legal values yourself: 11 through 15. Then consider object lifetime—the loop shown in comments and the active one-shot call do not exercise a `randc` history in the same way if a new generator object is repeatedly constructed.
